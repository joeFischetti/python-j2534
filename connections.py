...
class J2534Connection(BaseConnection):
    ...
    def __init__(
        self, windll, rxid, txid, name=None, debug=False, sniff=False, *args, **kwargs
    ):
        ...
        if sniff:
            ...
            self.logger.info("get stmin_tx")
            stmin = SCONFIG()
            stmin.Parameter = Ioctl_Parameters.STMIN_TX.value
            stmin.Value = ctypes.c_ulong(0xFFFF)
            self.result = self.interface.PassThruIoctl(
                Handle=self.channelID, IoctlID=Ioctl_ID.GET_CONFIG, ioctlInput=stmin
            )

            if self.result == Error_ID.ERR_SUCCESS:
                self.logger.info("Set STMIN_TX to 500")
            else:
                self.logger.info("Failed to set STMIN_TX to 500")
